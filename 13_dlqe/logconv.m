...
% float local_position[6]; //tangent plane mapping into x,y,z [m], vx,vy,vz

% int32_t gps_raw_position[3]; //latitude [degrees] north, longitude [degrees] east, altitude above MSL [millimeter]
% float attitude[3]; //pitch, roll, yaw [rad]
% float rotMatrix[9]; //unitvectors
% float vicon_pos[6]; //x,y,z in NED, roll, pitch, yaw

myPath = '.\';    %set relative path here
%myPath = '.\';                             
...
if exist(filePath, 'file')
    ...
    elements = int64(fileSize./(8+(3+3+3+1+1+1+4+8+1+3+6+3+3+9+6+4+6)*4))
    elements2 = elements;
    ...
    for i=1:elements
        ...
        % local position (6 channels)
        sensors(i,30:35) = fread(fid, 6, 'float', 0, 'ieee-le');
        
        % gps_raw_position (3 channels)
        sensors(i,36:38) = fread(fid, 3, 'uint32', 0, 'ieee-le');
        
        % attitude (3 channels)
        sensors(i,39:41) = fread(fid, 3, 'float', 0, 'ieee-le');
        
        % RotMatrix (9 channels)
        sensors(i,42:50) = fread(fid, 9, 'float', 0, 'ieee-le');
        
        % vicon position (6 channels)
        sensors(i,51:56) = fread(fid, 6, 'float', 0, 'ieee-le');
                
        % controll effective (4 channels)
        sensors(i,57:60) = fread(fid, 4, 'float', 0, 'ieee-le');
        
        % flow (6 channels)
        sensors(i,61:66) = fread(fid, 6, 'float', 0, 'ieee-le');
    end

...
x_aposteriori=[0;0;0];
P_aposteriori_k=eye(3)*800;

mode=1;

K_comupted=dlqe(A,eye(3),C,Q,R);

for i=1:length(z)
    if mode == 0
        x_apriori=A*x_aposteriori_k;
        P_apriori=A*P_aposteriori_k*A'+Q;
        %update
        x_measurement=z(i);
        yr=x_measurement-C*x_apriori;
        S=C*P_apriori*C'+R;
        K=(P_apriori*C')/S
        x_aposteriori=x_apriori+K*yr;
        P_aposteriori=(eye(3)-K*C)*P_apriori;
        x_aposteriori_k=x_aposteriori;
        P_aposteriori_k=P_aposteriori;
    else
        K = K_comupted;
        x_aposteriori=A*x_aposteriori+K*(z(i)-C*A*x_aposteriori);      
        %x_aposteriori_k=x_aposteriori;
        %x_aposteriori=A*x_aposteriori_k+K*(z(i)-C*A*x_aposteriori_k);      
        %x_aposteriori_k=x_aposteriori;
    end
    xp=[xp,x_aposteriori];   
end
display('precomputed K');
K_comupted
display('iterative K');
K
